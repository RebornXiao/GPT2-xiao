...
def generate(titles, k):
    ...
    save_path = 'generated_quantangshi/'
    device = "cuda" if torch.cuda.is_available() else "cpu"
    tokenizer = tokenization_bert.BertTokenizer('../cache/vocab_small.txt')
    model = GPT2LMHeadModel.from_pretrained('model_quantangshi/final_model')
    model.to(device)
    model.eval()
    ...
    with open(save_path + str(k) + '.txt', 'w', encoding='utf-8') as f:
        context_tokens = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(titles))
        generated = 0
        out = sample_sequence(
            n_ctx=n_ctx,
