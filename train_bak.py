...
class MyThread(threading.Thread):
    def __init__(self, i, all_len, lines, tokenized_data_path, num_pieces, full_tokenizer):
    ...
    def run(self):
        parse(self.i, self.all_len, self.lines, self.tokenized_data_path, self.num_pieces, self.full_tokenizer)
...
def build_files(data_path, tokenized_data_path, num_pieces, full_tokenizer, min_length):
    ...
    for filename in filenames:
        ...
        try:
            with open(os.path.join(data_path, filename), 'r', encoding='gbk') as f:
                nodel = f.readlines()
    ...
    # with open(data_path+'/黑魔导.txt', 'r', encoding='gbk') as f:
    #     print('reading lines')
    #     # lines = json.load(f)
    #     lines = f.readlines()
    ...
    for i in tqdm(range(num_pieces)):
        if i > 48:
            if not os.path.exists(tokenized_data_path + 'tokenized_train_{}.txt'.format(i)):
                t = MyThread(i, all_len, lines, tokenized_data_path, num_pieces, full_tokenizer)
                t.start()
            # parse(i, all_len, lines, tokenized_data_path, num_pieces, full_tokenizer)
            # t = threading.Thread(target=parse(i, all_len, lines, tokenized_data_path, num_pieces, full_tokenizer))
            # t.start()
            # threads.append(threading.Thread(target=parse(i, all_len, lines, tokenized_data_path, num_pieces, full_tokenizer)))
        #
        # sublinesNew = lines[all_len // num_pieces * i: all_len // num_pieces * (i + 1)]
        # if i == num_pieces - 1:
        #     sublinesNew.extend(lines[all_len // num_pieces * (i + 1):])  # 把尾部例子添加到最后一个piece
...
def main():
    ...
    parser.add_argument('--raw_data_path', default='data/nodel', type=str, required=False, help='原始训练语料')
    ...
    parser.add_argument('--num_pieces', default=402, type=int, required=False, help='将训练语料分成多少份')
